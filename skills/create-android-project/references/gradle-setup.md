...
> **How this maps onto the scaffolded project**
>
> Convention plugins are generated only for the **`clean-mvvm`** architecture.
> The **`mvvm`** architecture is a single module and applies plugins directly in
> `app/build.gradle.kts`, which is simpler and appropriate at that size.
>
> Three differences from the examples below:
>
> - Plugin ids use a prefix derived from the project name (`myapp.android.library`),
>   not `nowinandroid.*`.
> - The generated convention plugins configure `ApplicationExtension` and
>   `LibraryExtension` directly rather than the star-projected
>   `CommonExtension<*, *, *, *, *, *>`. The star count changes between AGP
>   versions, so avoiding it keeps `build-logic` compiling across upgrades.
> - There is a `jvm.library` convention for the `domain` module, which must stay
>   free of the Android SDK.
>
> The version catalog, `apply false` declarations in the root build file, and the
> `includeBuild("build-logic")` wiring all work exactly as described below.
