...
>
> The UI layer and data layer patterns below apply to **both** scaffolded
> architectures. The domain layer differs:
>
> - **`clean-mvvm`**: the domain layer is a required, pure-Kotlin `domain` module.
>   Repository interfaces live there and every ViewModel depends on use cases, not
>   on a repository. Use cases expose `operator fun invoke`.
> - **`mvvm`**: there is no domain layer. ViewModels depend on repositories
>   directly, and repository interfaces sit alongside their implementations in the
>   data package. This is intentional for small apps; see
>   `architecture-selection.md` for when to move.
>
> Naming in the scaffolded code follows `ItemRepository` / `ItemRepositoryImpl`
> and `toDomain()` / `toEntity()` mappers, rather than the `OfflineFirst…` prefix
> used in some examples below.
