...
>
> This guide applies to the **`clean-mvvm`** architecture. Two differences from
> the layout described below, because the scaffolded project follows Clean
> Architecture dependency inversion:
>
> | This guide | Scaffolded `clean-mvvm` |
> |---|---|
> | `core:model` holds domain models | `domain/model` holds them |
> | `core:data` holds repository interfaces *and* implementations | `domain/repository` holds the interfaces, `data/repository` the implementations |
> | feature `impl` depends on `core:data` | feature `impl` depends on `domain` (use cases) only |
> | convention plugin prefix `nowinandroid.*` | prefix derived from the project name, e.g. `myapp.*` |
>
> The **`mvvm`** architecture is a single module and does not use this layout at
> all; its packages mirror these boundaries without enforcing them. Everything
> below about low coupling, visibility and dependency direction still applies as
> guidance there.
